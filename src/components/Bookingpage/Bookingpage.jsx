...
import { google_api } from "../../../Constants.js"; // Import your API key from Constants.js
const Bookingpage = () => {
  ...
  const decrementor = () => setnumberofbag(Math.max(0, numberofbag - 1));
  ...
  const { isLoaded } = useJsApiLoader({
    ...
    googleMapsApiKey: `${google_api}`, // Insert your API key here
  });
    ...
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
          destination
        )}&key=${google_api}` // Insert your API key here
      );
